...
export default function SchulteGame() {
    ...
    useEffect(() => {
        try { const s = localStorage.getItem("nb_schulte_best_v2"); if (s) setBestTimes(JSON.parse(s)); } catch { }
    }, []);

    const startGame = useCallback((opt: typeof GRID_OPTIONS[0]) => {
        setGridOpt(opt);
        setGrid(buildGrid(opt.size));
        setNext(1); setWrongNum(null); setMistakes(0); setFinalTime(0); setRunning(false);
        timerRef.current?.reset();
        setPhase("playing");
    }, []);

    const handleCell = useCallback((num: number) => {
    ...
    const cellSize =
        gridOpt.size <= 3 ? "w-20 h-20 sm:w-24 sm:h-24 text-2xl sm:text-3xl" :
            gridOpt.size <= 4 ? "w-16 h-16 sm:w-18 sm:h-18 text-xl sm:text-2xl" :
                gridOpt.size <= 5 ? "w-12 h-12 sm:w-14 sm:h-14 text-base sm:text-lg" :
                    gridOpt.size <= 6 ? "w-10 h-10 sm:w-12 sm:h-12 text-sm sm:text-base" :
                        "w-9 h-9 sm:w-10 sm:h-10 text-xs sm:text-sm";

    const progressPct = Math.round(((next - 1) / (gridOpt.size * gridOpt.size)) * 100);
    ...
    return (
        <GameShell
            ...
            badge={
                phase !== "select" ? (
                    <span className="nb-badge" style={{ background: "rgba(56,189,248,0.1)", borderColor: "rgba(56,189,248,0.25)", color: "#38bdf8" }}>
                        {gridOpt.label}
                    </span>
                ) : undefined
            }
            ...
            {phase === "select" && (
                <div className="w-full max-w-md space-y-8 animate-nb-scale-in">
                    <div className="text-center">
                        ...
                        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">Schulte Table</h2>
                        <p className="text-sm sm:text-base" style={{ color: "rgba(255,255,255,0.45)" }}>
                            Click numbers in ascending order. Trains peripheral vision and sustained attention.
                        </p>
                    </div>
                    ...
                    <div className="space-y-2 nb-stagger">
                        {GRID_OPTIONS.map((opt) => (
                            <button key={opt.label} onClick={() => startGame(opt)}
                                ...
                                <span className="text-xl font-bold font-mono w-14 text-center nb-gradient-text-cool">{opt.label}</span>
                                <span className="flex-1 text-sm text-left" style={{ color: "rgba(255,255,255,0.45)" }}>{opt.desc}</span>
                                {bestTimes[opt.label] ? (
                                    <span className="text-xs text-emerald-400 font-mono font-semibold flex-shrink-0">
                                        🏆 {fmtMs(bestTimes[opt.label])}
                                    </span>
                                ) : (
                                    <span className="text-xs flex-shrink-0" style={{ color: "rgba(255,255,255,0.2)" }}>No record</span>
                                )}
                ...
                <div className="w-full flex flex-col items-center gap-4 sm:gap-6 animate-nb-scale-in">
                    ...
                    {phase === "playing" && (
                        <div className="flex items-center gap-3 sm:gap-4">
                            ...
                            <div className="nb-stat">
                                <span className="nb-stat-value text-white">{gridOpt.size * gridOpt.size}</span>
                                <span className="nb-stat-label">total</span>
                            </div>
                            <div className="nb-stat">
                                <span className={`nb-stat-value ${mistakes === 0 ? "text-emerald-400" : "text-red-400"}`}>{mistakes}</span>
                    ...
                    {/* Progress bar */}
                    {phase === "playing" && (
                        <div className="w-full max-w-sm h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                            <div
                                className="h-full rounded-full transition-all duration-300"
                                style={{ width: `${progressPct}%`, background: "linear-gradient(90deg, #38bdf8, #818cf8)" }}
                            />
                        </div>
                    )}
                    ...
                    {phase === "done" && (
                        <div className="text-center space-y-5 mt-2 animate-nb-bounce-in">
                            <div>
                                <p className="text-4xl font-bold nb-gradient-text mb-1">🏁 Complete!</p>
                                <p className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>
                                    {mistakes === 0 ? "🎯 Perfect run — no mistakes!" : `${mistakes} mistake${mistakes !== 1 ? "s" : ""}`}
                                </p>
                            </div>
                            <div className="nb-glass rounded-2xl px-8 py-4 inline-flex flex-col items-center gap-1">
                                <span className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Your time</span>
                                <span className="text-3xl font-mono font-bold text-amber-400">{fmtMs(finalTime)}</span>
                                {bestTimes[gridOpt.label] === finalTime && (
                                    <span className="text-emerald-400 text-xs font-semibold">✨ New Personal Best!</span>
                                )}
                            </div>
